...
const DEFAULT_CONFIG = {
  ...
  className: '',
  style: {},
  // 2. Added default grain configurations
  grain: false,
  grainOpacity: 0.1,
  grainSize: '150px' 
};
...
const GradualBlur = props => {
  ...
  // 3. Render the grain overlay conditionally
  return (
    <div
      ref={containerRef}
        ...
        {config.grain && (
          <div
            style={{
              ...
              backgroundSize: config.grainSize,
              opacity: config.grainOpacity,
              // Applies a gradient mask so the grain fades out smoothly alongside the blur
              maskImage: `linear-gradient(${getGradientDirection(config.position)}, transparent 0%, black 100%)`,
              WebkitMaskImage: `linear-gradient(${getGradientDirection(config.position)}, transparent 0%, black 100%)`,
            }}
          />
        )}
      </div>
